...
export const Projects = () => {
  ...
  const login = 'tienbot';
  const perPage = 100; // Количество проектов на странице

  useEffect(() => {
    // Выполните запрос к GitHub API, чтобы получить список проектов пользователя
    fetch(`https://api.github.com/users/${login}/repos?per_page=${perPage}`)
    ...
    <main className={s.main}>
      <div className={s.wrapper}>
        ...
        <Toggle
          dropdownContent={
            <Check
              setCheckedProjects={handleCheckedProjectsChange}
              setButtonXText={setButtonXText}
              setButtonXVisible={setButtonXVisible}
            />
          }
        >
      ...
      <div className={s.wrapperCards}>
        <div className={s.border}>
          {buttonXVisible && (
            <ButtonX onClick={handleButtonXClick}>{buttonXText}</ButtonX>
          )}
        </div>
