...
export const PageHello = () => {
  useEffect(() => {
    // Добавление обработчика события 'keydown' при монтировании компонента
    document.addEventListener("keydown", handleKeyDown);
    
    // Удаление обработчика события 'keydown' при размонтировании компонента
    return () => {
      document.removeEventListener("keydown", handleKeyDown);
    };
  }, []); // Пустой массив зависимостей, чтобы обработчик добавлялся и удалялся только один раз
  ...
  const handleKeyDown = (event) => {
    ...
    function clickButton(id) {
      setTimeout(() => {
        ...
        if (element) {
          element.style.borderColor = '#1E2D3D';
    ...
    <main className={s.pageHello}>
        <div className={s.pageHello__text}>
            <div className={s.pageHello__text_top}>
                ...
                <h1 className={s.pageHello__h1}>Aleksandr Plygun</h1>
                <p className={s.pageHello__textSubtext}>&gt;Front-end developer</p>
            </div>
            <div className={s.pageHello__text_bottom}>
                <p className={s.desktop}>&#47;&#47;complete the game and find a secret project!</p>
                ...
                <a className={s.pageHello__a} href="https://github.com/tienbot">“https://github.com/tienbot”</a>
                </p>
            </div>
        </div>
