...
function SnakeGame(){
  ...
  const draw = () => {
    ...
    //     // Прорисовка головы змейки (не забудь скобку на 150 строке)
    // if (index === 0) {
  ...
  const startGame = (event) => {
    if (event.target.matches('.Button_button__startGame__FTplJ') && event.target.textContent.includes('start game')){
        setGameStarted(true);
